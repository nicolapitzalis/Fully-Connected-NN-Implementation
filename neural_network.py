...
class NeuralNetwork():
    ...
    - activation_hidden (Callable[[np.ndarray], np.float32]): Activation function used in the hidden layers.
    - activation_hidden_prime (Callable[[np.ndarray], np.float32]): Derivative of the activation function used in the hidden layers.
    - activation_output (Callable[[np.ndarray], np.float32]): Activation function used in the output layer.
    - activation_output_prime (Callable[[np.ndarray], np.float32]): Derivative of the activation function used in the output layer.
    ...
    def _add_layer(self, input_size, output_size, activation=None, activation_prime=None):
        ...
        - activation (Callable[[np.ndarray], np.float32]): Activation function used in the layer.
        - activation_prime (Callable[[np.ndarray], np.float32]): Derivative of the activation function used in the layer.
        """
    ...
    def _network_architecture(self):
        ...
        for i in range(self.n_hidden_layers):
            if i == 0:
                self._add_layer(self.hidden_layer_sizes[i], self.n_features, self.activation_hidden, self.activation_hidden_prime)
            else:
                self._add_layer(self.hidden_layer_sizes[i], self.hidden_layer_sizes[i-1], self.activation_hidden, self.activation_hidden_prime)
        
        # Add output layer
        self._add_layer(self.hidden_layer_sizes[-1], self.n_output_units, self.activation_output, self.activation_output_prime)
        
    def _predict_ouptuts(self, features):
        """
